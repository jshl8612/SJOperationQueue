...
class SJOperationQueue: NSObject {
    ...
    typealias SJOperation = (_ onCompleted: @escaping () -> Void) -> Void
    ...
    init(syncOpQueueList: [SJOperationQueue]) {
        ...
        self.operateType = .sync
        self.queue = syncOpQueueList.map({ [weak self] op in
            
            let newOp: () -> Void = {
                let completed = op.onComplete
                op.onComplete = {
                    completed()
                    self?.next()
                }
    ...
    init(ayncOpQueueList: [SJOperationQueue]) {
        ...
        self.operateType = .async
        self.queue = ayncOpQueueList.map({ [weak self] op in
            let newOp: () -> Void = {
                let completed = op.onComplete
                op.onComplete = {
                    completed()
